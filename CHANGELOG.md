...
- 공모전 제출 패키지 초안(1페이지 시각화, 분석보고서 PDF·마크다운, 재배포 가능 CSV)을 생성한다
- 제출 초안의 페이지 수·출처·원천 제외·비밀값 부재를 검사한다

### Changed

- 출처 목록에 수집 방법을 넣고 접수용 ZIP과 Git 초안을 구분한다
...
- Git 워크플로·이슈 맵에서 연동 Secret과 브랜치 안내 중복을 빼고 연동 가이드로 모은다
- 국문 문서를 `docs/kor/`, 영문 문서를 `docs/eng/`로 나누고 표·체크섬은 `docs/data/`에 둔다
- Linear 프로젝트 슬러그를 YAML 문자열로 고정하고, 이슈 생성 키에 쓰기·프로젝트 권한을 요구한다

## [0.8.1] - 2026-08-26

### Fixed

- 배차 시각이 없는 노선을 서비스 기회 0으로 합산하던 문제를 고치고, 노선 정류장 CSV
  경로의 상위 폴더 생성과 2023 승하차 검증 해시 입력을 맞춘다
- 병합된 PR이 Linear 연동으로 다시 수정될 때 이미 닫힌 이슈를 재검사해 실패 알림이
  나던 문제를 고친다
...
- GitHub 커뮤니티 기준용 행동 강령, 기여 안내, MIT 라이선스, 보안 정책
...
- 데이터 문서의 CSV를 `docs/data/tables/`로 옮겨 마크다운과 표를 구분
- Keep a Changelog 반복 제목과 문서 표·줄길이 경고가 문제를 가득 채우지 않도록
  markdownlint 규칙을 맞춤
- 수집 권한이 열린 뒤에도 공간단위·기준일이 맞지 않는 자료는 점수에 넣지 않는다는 재검토를
  `docs/data/DATA_USAGE_REGISTER_2025.md`에 기록
...
- 행정동 대시보드에서 정책 예시를 하단에 분리하고, 지도를 낮춰 우측 근거 패널 높이를 맞춤
- 정책 카드를 선택한 동의 70점 분포 게이트에 따라 검토 후보·모니터링·상대 저취약으로 구분
- 폴더 역할을 `docs/PROJECT_STRUCTURE.md`에 수집·가공·분석·산출물로 구분해 적는다
...
- `busan_imd` 루트 분석·처리 모듈을 `analysis`와 `processing` 기능 패키지로
  재구성하고 문서·노트북 실행 경로를 동기화
...
- Clustering review, environmental double-burden overlay, and
  policy-priority matrix
- Interactive living-condition map spanning three major categories, eight
  child categories, and 13 indicators
...
- Added explicit estimation reasons and aligned the interface with the
...
- Domain- and indicator-level contribution analysis for 21 first-decile
  B-IMD priority areas
- Area-specific leading drivers based on weighted excess above citywide
  medians
...
- Extended the offline rebuild and bilingual methodology through
  priority-area explanations

### Fixed

- 큰 영역 가중치와 지역별 고유 취약 원인을 구분하도록 중앙값 대비 초과 기여도를 함께 제공
- Distinguished mechanical domain-weight effects from area-specific drivers
  using median-relative contributions
...
- Reproducible EDA, six-domain scoring, composite ranks and deciles for 206
  Busan dongs
...
- Extended the offline rebuild and bilingual Wiki through the
  sensitivity-analysis stage

### Fixed

- 완료 알림이 GitHub Issue Closed와 Linear Done을 모두 확인한 뒤에만 Slack으로 전송되도록 강화
- Ensured Slack completion notices are sent only after GitHub and Linear
...
- Docker-based Jupyter geospatial environment with Python packaging, tests,
  and CI
...
- Converted repository-relative documentation links into valid Wiki or
  source-file URLs
